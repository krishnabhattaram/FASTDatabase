...
;-
pro get_2dt_ts_antispacecraftvel_integration,funct,get_dat, $
  T1=t1, $
  T2=t2, $
  ENERGY=en, $
  ERANGE=er, $
  ...
  CALIB = calib, $
  n_get_pts = n_get_pts, $
  ANTISPACECRAFTVEL_INTEGRATION = antivel_int, $
  OUTFLOW = outflow, $
  INFLOW = inflow, $
  latitude=lat
  ...
  if ((keyword_set(antivel_int) or keyword_set(outflow)) and not keyword_set(lat)) then begin
    print, 'Need to include latitude data for latitude-specific integration!' 
  endif
  ...
  while (dat(arr_idx).valid ne 0) and (n lt max) do begin
    if (dat(arr_idx).valid eq 1) then begin
      ...
      if keyword_set(antivel_int) or keyword_set(outflow) or keyword_set(inflow) then begin
        closest_time_index = value_locate_error(time(n), lat.x, 10) ; Find closest latitude data within 10 s
        
        ; Restrict northern and southern hemisphere pitch angles to conduct integration over required ranges
        if keyword_set(antivel_int) then an_n = [0, 180] else if keyword_set(outflow) then an_n = [90, 180] else an_n = [0, 90]
        if keyword_set(antivel_int) then an_s = [180, 360] else if keyword_set(outflow) then an_s = [270, 360] else an_s = [180, 270]
        if lat.y[closest_time_index] lt 0 then an = an_s else an = an_n
        
        if closest_time_index lt 0 then sum = missing else sum = 2 * call_function(funct,dat(arr_idx),ENERGY=en,ERANGE=er,EBINS=ebins,ANGLE=an,ARANGE=ar,BINS=bins)
      endif else begin
